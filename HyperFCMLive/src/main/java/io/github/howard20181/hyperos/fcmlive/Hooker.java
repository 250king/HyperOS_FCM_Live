...
public class Hooker extends XposedModule {
    ...
    private void hookSystemServer(ClassLoader classLoader) {
        ...
        try {
            ...
            log(Log.ERROR, TAG, "Failed to hook Aurogon no-restrict", e);
        }
    }
    ...
    private void hookPackage(String packageName, ClassLoader classLoader) {
        if ("com.miui.powerkeeper".equals(packageName)) {
            ...
            try {
                hookPowerKeeperNoRestrict(classLoader);
            } catch (Exception e) {
                log(Log.ERROR, TAG, "Failed to hook PowerKeeper no-restrict", e);
            }
        }
    }
    ...
    /**
     * PowerKeeper is the producer of MILLET_NO_RESTRICT_APP.
     *
     * PowerKeeper 4.2.00 calls UserConfigureHelper#getNoRestrictApps(Context)
     * from ActiveStateController#dealNoRestrictApp(), then serializes that list
     * into Settings.System.MILLET_NO_RESTRICT_APP.
     *
     * Returning a copy with GMS appended fixes the source of truth directly:
     * user-selected no-restrict packages are preserved and there is no polling,
     * extra Settings write, or race with PowerKeeper regeneration.
     */
    private void hookPowerKeeperNoRestrict(ClassLoader classLoader)
            throws ClassNotFoundException, NoSuchMethodException {
        var helperClass = classLoader.loadClass(
                "com.miui.powerkeeper.provider.UserConfigureHelper"
        );
        var getNoRestrictApps = helperClass.getDeclaredMethod(
                "getNoRestrictApps",
                Context.class
        );

        Utils.evaluate(
                hook(getNoRestrictApps),
                h -> setHookId(h, "PK_getNoRestrictApps")
        ).intercept(chain -> {
            var result = chain.proceed();
            ...
            var fixed = new java.util.ArrayList<String>(source.size() + 1);
            for (Object item : source) {
                if (item instanceof String pkg && !fixed.contains(pkg)) {
                    fixed.add(pkg);
                }
            }

            if (!fixed.contains(GMS_PACKAGE_NAME)) {
                fixed.add(GMS_PACKAGE_NAME);
                log(Log.INFO, TAG,
                        "PowerKeeper no-restrict: appended GMS before MILLET write");
            }
            return fixed;
        });
        deoptimize(getNoRestrictApps);
    }

    /**
     * Consumer-side fallback in system_server.
     *
     * AurogonImmobulusMode#getNoRestrictApps() parses
     * MILLET_NO_RESTRICT_APP into mNoRestrictAppSet. This set is consulted by
     * Xiaomi's Aurogon/Greezer freeze policy, including the ordinary freeze path
     * that is independent of triggerGMSLimitAction().
     */
    private void hookAurogonNoRestrict(ClassLoader classLoader)
            throws ClassNotFoundException, NoSuchMethodException, NoSuchFieldException {
        var aurogonClass = classLoader.loadClass(
                "com.miui.server.greeze.AurogonImmobulusMode"
        );

        var noRestrictSetField = aurogonClass.getDeclaredField("mNoRestrictAppSet");
        noRestrictSetField.setAccessible(true);

        var getNoRestrictApps = aurogonClass.getDeclaredMethod("getNoRestrictApps");
        getNoRestrictApps.setAccessible(true);
        Utils.evaluate(
                hook(getNoRestrictApps),
                h -> setHookId(h, "Aurogon_getNoRestrictApps")
        ).intercept(chain -> {
            var result = chain.proceed();
            try {
                Object raw = noRestrictSetField.get(chain.getThisObject());
                if (raw instanceof Set<?> source && !source.contains(GMS_PACKAGE_NAME)) {
                    var fixed = new HashSet<Object>(source);
                    fixed.add(GMS_PACKAGE_NAME);
                    noRestrictSetField.set(chain.getThisObject(), fixed);
                    log(Log.INFO, TAG,
                            "Aurogon no-restrict: appended GMS to mNoRestrictAppSet");
                }
            } catch (Throwable t) {
                log(Log.ERROR, TAG,
                        "Failed to update Aurogon mNoRestrictAppSet", t);
            }
            return result;
        });
        deoptimize(getNoRestrictApps);

        var isNoRestrictApp = aurogonClass.getDeclaredMethod(
                "isNoRestrictApp",
                String.class
        );
        isNoRestrictApp.setAccessible(true);
        Utils.evaluate(
                hook(isNoRestrictApp),
                h -> setHookId(h, "Aurogon_isNoRestrictApp")
        ).intercept(chain -> {
